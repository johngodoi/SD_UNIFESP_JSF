...
$(document).ready(function() {
	...
	if(form_atual) {
		...
		form.id = 'Meu formulário';
		form.usuario = 'Meu usuário';
	...
	$('#gravar').click(function() {
		...
		elemento.proximo = $('#campo_proximo').val();

		switch(elemento.tipo){
			case 'text':
				elemento.texto_padrao = $('#campo_texto_padrao').val();
				elemento.tamanho = $('#campo_tamanho').val();
				break;
			case 'number':
				elemento.valor_padrao = $('#campo_valor_padrao').val();
				elemento.max = $('#campo_valor_max').val();
				elemento.min = $('#campo_valor_min').val();
				break;
			case 'radio':
			case 'checkbox':
	...
	$('#gravar_form').click(function() {
		form.id = $('#id_form').val();
		form.usuario = $('#id_usuario').val();
		form.elementos[form.elementos.length-1].proximo = -1;
		form.desenhar();
		$('li#campo_' + elemento_em_edicao).click();	
	...
	$('#limpar_form').click(function() {
		...
		$('#id_form').val("Meu formulário");
		$('#id_usuario').val("Meu usuário");
...
function editaCampo(id) {
	...
	var proximo = elemento.proximo;
	if(proximo == ''){
		var proximo = parseInt(id,10)+1;
		elemento.proximo = proximo;
	}
	$('#campo_proximo').val(elemento.proximo);

	switch(elemento.tipo){
		case 'text':
			$('#propriedades_text').show();
			$('#campo_texto_padrao').val(elemento.texto_padrao);
			$('#campo_tamanho').val(elemento.tamanho);
			break;
		case 'number':
			$('#propriedades_number').show();	
			$('#campo_valor_padrao').val(elemento.valor_padrao);
			$('#campo_valor_max').val(elemento.max);
			$('#campo_valor_min').val(elemento.min);
			break;
		case 'radio':
		case 'checkbox':
